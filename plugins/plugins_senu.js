...
const yourName = "*POWERED by QUEEN-SENU-MD*"; // YOURBOTNAME 💚 කියන තැනට ඔයාගේ බොට්ගේ නම හරි ඔයාගෙ නම හරි දාන්න.
...
*╎*
*╰───────────────◦•◦❥•*\n\n\> *POWERED by BLACKY-X-MD*`

await conn.sendMessage(from,{image: {url: data.datalist.list[0].icon},caption: desc},{quoted: mek})
await conn.sendMessage(from,{document: {url: data.datalist.list[0].file.path_alt},fileName: data.datalist.list[0].name,mimetype: 'application/vnd.android.package-archive',caption: `> *POWERED by BLACKY-X-MD*`},{quoted: mek})
...
async(conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/BLACKY-X-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        let message = {
            header: 'BLACKY-X-MD SIHALASUB.LK MOVIE SEARCH',
            footer: '> *POWERED by BLACKY-X-MD*',
...
`;
        ...
        conn.ev.on('messages.upsert', async (messageUpdate) => {
            ...
            if (isReplyToSentMsg) {
                ...
                await conn.sendMessage(from, { react: { text: '⬆️', key: mek.key } });
                if (messageType === '1') {
                    await conn.sendMessage(from, { document: { url: directLink }, mimetype: "video/mp4", fileName: `🎬 BLACKY-X-MD 🎬\n${sss?.result?.title ?? 'Movie'}.mkv`, caption: `> ${sss?.result?.title ?? 'Movie'}\n\n> 480p\n\n> *POWERED by BLACKY-X-MD*` }, { quoted: mek });
                } else if (messageType === '2') {
                    await conn.sendMessage(from, { document: { url: directLink1 }, mimetype: "video/mp4", fileName: `🎬 BLACKY-X-MD 🎬\n${sss?.result?.title ?? 'Movie'}.mkv`, caption: `> ${sss?.result?.title ?? 'Movie'}\n\n> 720p\n\n> *POWERED by BLACKY-X-MD*` }, { quoted: mek });
                } else if (messageType === '3') {
                    await conn.sendMessage(from, { document: { url: directLink2 }, mimetype: "video/mp4", fileName: `🎬 BLACKY-X-MD 🎬\n${sss?.result?.title ?? 'Movie'}.mkv`, caption: `> ${sss?.result?.title ?? 'Movie'}\n\n> 1080p\n\n> *POWERED by BLACKY-X-MD*` }, { quoted: mek });
                }
...
async(conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/BLACKY-X-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        const config = await readEnv();
        const data = await fetchJson(`https://viper-moviedl.vercel.app/api/cinesubz/search?q=${q}&apikey=asitha2005`)
...
*➟➟➟➟➟➟➟➟➟➟➟➟➟➟*
▫️🔗 *Join :* https://whatsapp.com/channel/0029Vb6Aa8bKmCPSdBJ25K2y
*➟➟➟➟➟➟➟➟➟➟➟➟➟➟*
...
if (q.includes("https://cinesubz.co/movies")) {
        const config = await readEnv();
        const data2 = await fetchJson(`https://viper-moviedl.vercel.app/api/cinesubz/movie?url=${q}&apikey=viper2007`)
...
const _1080 = await fetchJson(`https://viper-moviedl.vercel.app/api/cinesubz/download?url=${link[2]}&apikey=viper2007`)
const _720 = await fetchJson(`https://viper-moviedl.vercel.app/api/cinesubz/download?url=${link[1]}&apikey=viper2007`)
const _480 = await fetchJson(`https://viper-moviedl.vercel.app/api/cinesubz/download?url=${link[0]}&apikey=viper2007`)
    ...
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/BLACKY-X-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        const titell = q.split(" & ")[1] 
         ...
         const linnk = await fetchJson(`https://viper-moviedl.vercel.app/api/cinesubz/episode?url=${linkk}&apikey=viper2007`)
    ...
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/BLACKY-X-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        await conn.sendMessage(from, { 
            ...
            caption: `\n*${titel}*\n\n> *${si}*\n\n> *POWERED by BLACKY-X-MD*`,
            mimetype: "video/mp4",
            fileName: `🎬 BLACKY-X-MD 🎬${titel}.mp4`
        });
...
async(conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/BLACKY-X-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        const config = await readEnv();
        const data = await fetchJson(`https://mr-viper.vercel.app/movie/firemovie/search?text=${q}`)
...
if (q.includes("https://firemovieshub.com/tvshows")) {
                const configs = await readEnv();
		const tvdata2 = await fetchJson(`https://mr-viper.vercel.app/movie/firemovie/tvshow?url=${q}`)
let buttonns = [{
    name: "single_select",
    buttonParamsJson: JSON.stringify({
        title: 'Select One Episode :)',
...
if (q.includes("https://firemovieshub.com/movies")) {
        ...
        const data2 = await fetchJson(`https://mr-viper.vercel.app/movie/firemovie/movie?url=${q}`)
